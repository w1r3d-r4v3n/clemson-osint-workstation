...
runuser -u "${STUDENT}" -- env \
  ...
  python3 "${SCRIPT_ROOT}/bin/osint-case" new "Managed audit CI" --purpose "Synthetic integration test"

cases=("${STUDENT_HOME}"/Cases/*)
CASE_PATH="${cases[0]}"
SUDO_USER="${STUDENT}" python3 "${SCRIPT_ROOT}/bin/osint-audit-anchor" "${CASE_PATH}"

runuser -u "${STUDENT}" -- env \
  HOME="${STUDENT_HOME}" \
  OSINT_CASES_DIR="${STUDENT_HOME}/Cases" \
  OSINT_MANAGED_MARKER=/does-not-exist \
  python3 "${SCRIPT_ROOT}/bin/osint-case" source "${CASE_PATH}" https://example.org/ --title Example
SUDO_USER="${STUDENT}" python3 "${SCRIPT_ROOT}/bin/osint-audit-anchor" "${CASE_PATH}"

runuser -u "${STUDENT}" -- python3 "${SCRIPT_ROOT}/bin/osint-audit-verify" "${CASE_PATH}"
sed -i 's/source-logged/source-erased/' "${CASE_PATH}/audit.jsonl"
if runuser -u "${STUDENT}" -- python3 "${SCRIPT_ROOT}/bin/osint-audit-verify" "${CASE_PATH}"; then
